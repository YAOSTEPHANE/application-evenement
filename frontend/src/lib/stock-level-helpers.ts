...
/** Niveaux effectifs (cache local / anciennes données sans `stockLevels`). */
export function stockLevelsForArticle(article: {
  stockLevels?: Partial<StockLevels> | null;
  seuilMin?: number;
}): StockLevels {
  const raw = article.stockLevels;
  if (raw && typeof raw === "object") {
    return normalizeStockLevels({
      ...raw,
      min: raw.min ?? article.seuilMin ?? 0,
    });
  }
  return normalizeStockLevels({ min: article.seuilMin ?? 0 });
}

export function stockLevelsForVariant(variant: {
  stockLevels?: Partial<StockLevels> | null;
  seuilMin?: number;
}): StockLevels {
  const raw = variant.stockLevels;
  if (raw && typeof raw === "object") {
    return normalizeStockLevels({
      ...raw,
      min: raw.min ?? variant.seuilMin ?? 0,
    });
  }
  return normalizeStockLevels({ min: variant.seuilMin ?? 0 });
}
