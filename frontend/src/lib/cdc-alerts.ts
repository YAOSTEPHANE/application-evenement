...
export async function runCdcScheduledAlerts(organizationId: string): Promise<CdcAlertRunResult> {
  ...
  const staleBt = await prisma.stockDocument.findMany({
    where: {
      ...
      btEmittedAt: { lt: fortyEightHoursAgo },
    },
  ...
  const overdueBtDisputes = await prisma.stockDocument.findMany({
    where: {
      organizationId,
      kind: StockDocumentKind.BT,
      status: StockDocumentStatus.DISPUTED,
      transferDisputeDeadline: { lt: now },
    },
    select: { id: true, documentNumber: true },
  });

  for (const doc of overdueBtDisputes) {
    disputedAlerts += 1;
    await notifyRoleGroup(prisma, organizationId, ["ADMIN"], {
      module: "alertes",
      title: "Litige transfert > 48 h",
      body: `${doc.documentNumber} — arbitrage administrateur requis (§7.4.2).`,
      targetType: "StockDocument",
      targetId: doc.id,
      severity: "URGENT",
    });
  ...
  for (const doc of disputed) {
    disputedAlerts += 1;
    const isBtTransfer = await prisma.stockDocument.findFirst({
      where: { id: doc.id, kind: StockDocumentKind.BT, transferDisputeOpenedAt: { not: null } },
      select: { id: true },
    });
    if (isBtTransfer) continue;
