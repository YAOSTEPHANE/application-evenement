...
export const updateStockDocumentMetaSchema = z.object({
  shipperUserId: objectId.optional().nullable(),
  receiverUserId: objectId.optional().nullable(),
  driverUserId: objectId.optional().nullable(),
  sourceReference: z.string().max(120).optional().nullable(),
  anomalyNotes: z.string().max(2000).optional().nullable(),
});

export const updateDocumentLineConditionSchema = z.object({
  lineId: objectId,
  lineCondition: z.nativeEnum(ReturnCondition),
});
...
const documentInclude = {
  ...
  archive: { select: { retentionUntil: true, contentHash: true, createdAt: true } },
  correctsDocument: { select: { id: true, documentNumber: true } },
  rectificatoryOf: {
...
function enforceStockDocumentMutable(status: StockDocumentStatus): void {
  const check = assertStockDocumentMutable(status);
  if (!check.ok) {
    throw new StockDocumentDbError(check.message, 409);
  }
}
...
async function applySignedDocumentEffects(
  ...
  if (
    signed.kind === StockDocumentKind.BE &&
    signed.beSubtype !== BeSubtype.BE_RET &&
...
export async function updateStockDocumentMeta(
  organizationId: string,
  documentId: string,
  raw: unknown,
) {
  const payload = updateStockDocumentMetaSchema.parse(raw);
  const doc = await prisma.stockDocument.findFirst({
  ...
  enforceStockDocumentMutable(doc.status);
  return prisma.stockDocument.update({
    where: { id: documentId },
    data: {
      ...(payload.shipperUserId !== undefined ? { shipperUserId: payload.shipperUserId } : {}),
      ...(payload.receiverUserId !== undefined ? { receiverUserId: payload.receiverUserId } : {}),
      ...(payload.driverUserId !== undefined ? { driverUserId: payload.driverUserId } : {}),
      ...(payload.sourceReference !== undefined
        ? { sourceReference: payload.sourceReference }
        : {}),
      ...(payload.anomalyNotes !== undefined ? { anomalyNotes: payload.anomalyNotes } : {}),
    },
    include: documentInclude,
  });
}

export async function updateDocumentLineCondition(
  organizationId: string,
  documentId: string,
  raw: unknown,
) {
  const payload = updateDocumentLineConditionSchema.parse(raw);
  const doc = await prisma.stockDocument.findFirst({
    where: { id: documentId, organizationId, kind: StockDocumentKind.BE },
    include: { lines: true },
  });
  if (!doc) {
    throw new StockDocumentDbError("Bon BE introuvable", 404);
  }
  enforceStockDocumentMutable(doc.status);
  ...
  await prisma.documentLine.update({
    ...
    data: { lineCondition: payload.lineCondition },
  });
  ...
  return prisma.$transaction(async (tx) => {
      ...
      data: {
        ...
        anomalyNotes: mismatch
          ? isBt && btReceptionPhase
            ? `Écart transfert expédié/reçu — litige §7.4.2 (${unknownTags.join(", ")})`
    ...
    if (mismatch) {
      ...
      await notifyRoleGroup(tx, organizationId, roles, {
        ...
        body: `${doc.documentNumber} — vérification requise avant signature`,
        targetType: "StockDocument",
        targetId: doc.id,
        severity: "URGENT",
      });
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (refreshed.kind === StockDocumentKind.BT && signedCount === 1 && signedCount < needed) {
      const emitCheck = assertBtMandatoryFieldsForSignature({
        ...
        btSubtype: refreshed.btSubtype,
        btTransitPhase: refreshed.btTransitPhase,
    ...
    if (refreshed.kind === StockDocumentKind.BT && signedCount >= needed) {
      ...
      if (btTransferQuantityDiscrepancy(refreshed.lines)) {
        ...
        await notifyRoleGroup(tx, organizationId, ["ADMIN"], {
          ...
          body: `${refreshed.documentNumber} — écart expédié/reçu à trancher sous 48 h`,
          targetType: "StockDocument",
        ...
        return tx.stockDocument.update({
          ...
          data: {
            ...
            anomalyNotes:
              "Litige transfert — écart quantité expédiée (scan départ) / reçue (scan arrivée).",
          },
          include: documentInclude,
        });
      }
...
export async function cancelStockDocument(
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (doc.status === StockDocumentStatus.SIGNED) {
      throw new StockDocumentDbError(
        "Bon signé — suppression interdite (§7.5). Utilisez la contre-passation (bon contraire) ou un bon rectificatif.",
        409,
      );
    }
    return tx.stockDocument.update({
...
/** Portique : valide les tags contre un BS (sortie) ou BE (entrée) ouvert selon le sens du passage. */
export async function validatePortalScan(
  ...
  /** §7.3.3 — sortie portique : priorité BS-EVT ouvert, rapprochement liste théorique. */
...
/**
 * Chargement : génère automatiquement le BS-EVT lié à la commande (CDC interdépendance).
 * Idempotent si un BS-EVT ouvert existe déjà.
 */
...
/**
 * Retour prestation : génère automatiquement le BE-RET (reprise du BS-EVT signé).
 */
...
/** §7.4.2 phase 3 — arbitrage administrateur d'un litige de transfert BT. */
export async function resolveBtTransferDispute(
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (doc.signatures.length < 2) {
      throw new StockDocumentDbError(
        "Double validation incomplète — signatures émission et réception requises",
        409,
      );
    }
...
/** §7.5 — bon rectificatif lié à un bon signé (correction sans modifier l'original). */
export async function createRectificatoryDocument(
  ...
  if (actorRole !== "ADMIN" && actorRole !== "MANAGER") {
    throw new StockDocumentDbError("Bon rectificatif réservé à l'administration", 403);
  }
  const trimmed = reason.trim();
  if (trimmed.length < 3) {
    throw new StockDocumentDbError("Motif du rectificatif requis (3 caractères min.)", 400);
  }

  const original = await getStockDocument(organizationId, documentId);
  if (original.status !== StockDocumentStatus.SIGNED) {
    throw new StockDocumentDbError(
      "Seul un bon signé peut faire l'objet d'un rectificatif (§7.5).",
      400,
    );
  }
